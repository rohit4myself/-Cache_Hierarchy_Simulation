...
{     list <int> lines2;
     ...
     public :
       ...
       void lru3Invalidate(int set,int way);
...
//method for Invalidating from L2 
void Lru::lru2Invalidate(int set,int way)
{
   sets2.at(set).remove(way);
   sets2.at(set).push_back(way);
}
//method for Invalidating from L3 
void Lru::lru3Invalidate(int set,int way)
{
   sets3.at(set).remove(way);
   sets3.at(set).push_back(way);
}
// End of LRU -----------------------------------------------------------------------------------------------------------
...
//Function to insert a block in L2 cache (block which is evicted from L2 cache)
void exclusion_policy_insert_inl3(unsigned long long address){
    //Obtaining L3 cahce address_index and tag for this received address
    unsigned long long block_number;
    block_number = address / BLOCK_SIZE;
    int address_index = int(block_number % 2048);
    
    //Getting the line at which this new block/address is need to be placed
    int replace_line = lru.lru3GetLine(address_index);
    //Placing/Inserting this new block in this replace_line obtained
    l3cache[address_index][replace_line] = address;
    lru.lru3Update(address_index,replace_line);
    
}

// Function to insert a block in L2 cache on L2 cache miss(either from L3 cache or from M.M.) 
void exclusion_policy_insert_in_l2(unsigned long long address){
    //Obtaining tag and address_index for this address
    unsigned long long block_number;
    block_number = address / BLOCK_SIZE;
    int address_index = int(block_number % 1024); 
    
    //Getting Line which needs to be replaced
    int replaced_line = lru.lru2GetLine(address_index);
    //Storing replaced line block/address into L3 cache 
    unsigned long long replaced_address = l2cache[address_index][replaced_line];
    exclusion_policy_insert_inl3(replaced_address);
    
    //Inserting address (recieved parameter to this function) in L2 cache
    l2cache[address_index][replaced_line] = address;
    lru.lru2Update(address_index, replaced_line);
}
	...
	int count =0;
     for( int line =0 ; line < 16; line++){	
        if(block_number == (l3cache[address_index][line]/BLOCK_SIZE)) {
            ...
            l3_hit++;
            //invalidate the hit block in L3 as it will be transferred to L2 Cache
            l3cache[address_index][line] =0;
            lru.lru3Invalidate(address_index,line);
            //inserting the block in L2 Cache
            exclusion_policy_insert_in_l2(address);
            break;
        }
...
// Printing output 
cout<<"Cache Policy: Exclusion"<<endl;
